...
<script>
	...
	function runButton() {
						if ( cont == 0 ){							
							...
							document.getElementById('warning_message').innerHTML = '<?php echo JText::_( 'COM_SECURITYCHECK_FILEMANAGER_WARNING_MESSAGE' ); ?>';
						} else if ( cont == 1 ){
							url = 'index.php?option=com_securitycheck&controller=filemanager&format=raw&task=acciones_clear_data';
							etiqueta = 'current_task';
							
						} else {
							url = 'index.php?option=com_securitycheck&controller=filemanager&format=raw&task=getEstadoClearData';
...
<div class="securitycheck-bootstrap">
	<div id="buttonwrapper" class="buttonwrapper">
		<button class="btn btn-primary" type="button" onclick="hideElement('buttonwrapper'); runButton();"><i class="icon-fire icon-white"></i><?php echo JText::_( 'COM_SECURITYCHECK_CLEAR_DATA_CLEAR_BUTTON' ); ?></button>
	</div>
</div>
