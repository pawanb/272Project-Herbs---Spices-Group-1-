...
*/
class SecuritychecksModelSecuritychecks extends JModelLegacy
{
/**
* Array de datos
* @var array
		...
		foreach ($vuln_components as $indice2){
			...
			if ($nombre == $nombre_vuln){  // El componente es vulnerable, chequeamos la versión del producto y la de Joomla 
				...
				if ($componente_vulnerable){ //La versión del componente es vulnerable; chequeamos si lo es para nuestra versión de Joomla
					$modvulnjoomla = $vuln_components[$j]['modvulnjoomla']; //Modificador sobre la versión de Joomla
					$vuln_joomla_version = $vuln_components[$j]['Joomlaversion']; // Versión de Joomla para la que es vulnerable el componente
					...
					/* Nos aseguramos que el componenete tiene el valor "No" en el campo "Vulnerable". Esto es útil cuando se cambia la versión
					del componente y pasa de 'vulnerable' o 'Notdefined' a 'no vulnerable' */
