...
class plgjsnimageshowThemeFlowInstallerScript
{
	...
	private function _updateSchema()
	{
		...
		$eid = $row->find(array('element' => 'themeflow', 'type' => 'plugin'));
		if ($eid)
		{
			$db = JFactory::getDBO();
	...
	private function _getInfo($id)
	{
		...
		$query->select('*')
			...
			->where('element=\'themeflow\' AND type=\'plugin\' AND folder=\'jsnimageshow\' AND extension_id = ' . $id);
		$db->setQuery($query);
