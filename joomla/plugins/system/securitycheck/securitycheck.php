...
class plgSystemSecuritycheck extends JPlugin{
	function plgSystemSecuritycheck( &$subject, $config ){
	...
	function cleanQuery($ip,$string,$methods_options,$a,$request_uri,&$modified,$check,$option){
		...
		if ( (!(is_array($string))) && (mb_strlen($string)>0) && ($pageoption != '') ){
			...
			if ( !(strstr($strip_tags_exceptions,$pageoption)) && !(strstr($strip_tags_exceptions,'*')) ){
			...
			if (!$modified) {
				...
				if ( !(strstr($sql_pattern_exceptions,$pageoption)) && !(strstr($sql_pattern_exceptions,'*')) ){						
				...
				if ( !(strstr($if_statement_exceptions,$pageoption)) && !(strstr($if_statement_exceptions,'*')) ){	
			...
			/* LFI  Prevention */
			$lfiStatements = array("/\.\.\//");
			if ( !(strstr($lfi_exceptions,$pageoption)) && !(strstr($lfi_exceptions,'*')) ){
				if (!$modified) {
	...
	function second_level($request_uri,$string,$a,&$found,$option){
		...
		if (!($user->authorise('com_installer'))) { 
			if ( ( !($is_admin) ) && ($pageoption != '') && !(is_array($string)) ){  // No estamos en la parte administrativa
				if ( !(strstr($second_level_exceptions,$pageoption)) ){
					/* SQL Injection Prevention */
					// Prevents duplicate backslashes
					if(get_magic_quotes_gpc()){ 
						$string_sanitized = stripslashes($string);
