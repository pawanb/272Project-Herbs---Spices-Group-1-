 ...
 * Plugin for Joomla 3.0 - Version 1.2.1
 * License: http://www.gnu.org/copyleft/gpl.html
 * Authors: marco maria leoni
 * Copyright (c) 2010 - 2013 marco maria leoni web consulting - http: www.mmleoni.net
 * Project page at http://www.mmleoni.net/sql-iniection-lfi-protection-plugin-for-joomla
 * *** Last update: Apr 5th, 2013 ***
*/
	...
	function onAfterInitialise(){
			...
			try{
				...
				if ($e->getCode() == 1146){
					try{
						...
						JError::raiseError(500, 'CAN\'T CREATE IPTABLE. MYSQL ERROR ' . $e->getCode() . ' : '.$e->getMessage() );
					}
				}else{
					JError::raiseError(500, 'UNEXPECTED MYSQL ERROR ' . $e->getCode() . ' ACCESSING IPTABLE: '.$e->getMessage() );
				}
			}
		...
		foreach(explode(',', $p_nameSpaces) as $nsp){
			...
			foreach($nameSpace as $k => &$v){
				...
				/* LFI */
				if ($p_strictLFI){
					if (!in_array($k, array('controller', 'view', 'model', 'template', 'tpl'))) continue;
				}
